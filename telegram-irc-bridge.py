...
# telegram-irc-bridge
bridgeVersion = "0.1.2" # don't comment this out
...
def bridge_alltext(update, context):
	...
	if toIrcText.startswith("/me "):
		...
		if actionText == None:
			return None
		toIrcText = ctcpChar + "ACTION " + " ".join(actionText) + ctcpChar
			...
			for rawline in data:
				...
				if line[0] == "PING": # ping responses, the bread and butter of any ircd
					...
					if line[2].startswith("\x01ACTION") or line[2].startswith(":\x01ACTION"):
						...
						sendToTelegramChat( destinationChatId, newtext, False )
						printLog(" * IRC " + messageType + " " + str(line[1]).lower(), " * " + ircuser["nick"] + " " + " ".join(line[3:]) )
					else:
						newtext = " ".join(line[2:])[1:] # remove leading colon
						sendToTelegramChat( destinationChatId, newtext )
						printLog(" * IRC " + messageType + " " + str(line[1]).lower(), "<" + ircuser["nick"] + "> " + newtext )
